...
// Mirrors app/reporting/page.tsx's exact gate structure — Model Portfolios
// reuses the "reporting" feature area/role wholesale (no new role
// vocabulary), so the org/role check hits the same GET /api/reporting/organization
// endpoint Reporting itself uses.
  ...
  return (
    <main className="flex flex-1 flex-col items-center gap-6 p-8 pt-16">
      ...
      {loadingOrg || !orgState ? (
        ...
        <ModelPortfoliosWorkspace isAdmin={orgState.role === "admin"} />
      )}
    </main>
  );
}
